...
>(({ className, ...props }, ref) => (
  <span
    className={cn(
      "size-full absolute group-hover:bg-neutral-200  group-hover:desktop:size-9 group-hover:laptop:size-9 group-hover:size-[45px] group-hover:z-[-1] group-hover:absolute group-hover:rounded-full",
      className
    )}
    ref={ref}
    {...props}
...
>(({ className, weekdays = weekChn, ...props }, ref) => {
  const baseStyle =
    "border-b border-b-stone-300 min-w-10 w-[14%] text-center text-sm text-slate-300 grow pb-1 justify-between";
  return (
    <>
